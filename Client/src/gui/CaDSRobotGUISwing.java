...
public class CaDSRobotGUISwing implements ICaDSRobotGUIUpdater {
	private CaDSRobotControlGUISwing controller;
	private Thread refreshThread;
	private final ICaDSRMIConsumer consumer;

	public CaDSRobotGUISwing(ICaDSRMIConsumer paramICaDSRMIConsumer,
			IIDLCaDSEV3RMIMoveGripper paramIIDLCaDSEV3RMIMoveGripper,
			IIDLCaDSEV3RMIMoveVertical paramIIDLCaDSEV3RMIMoveVertical,
			IIDLCaDSEV3RMIMoveHorizontal paramIIDLCaDSEV3RMIMoveHorizontal,
			IIDLCaDSEV3RMIUltraSonic paramIIDLCaDSEV3RMIUltraSonic) {
		this.consumer = paramICaDSRMIConsumer;
		this.refreshThread = null;
		this.controller = new CaDSRobotControlGUISwing(paramICaDSRMIConsumer,
				paramIIDLCaDSEV3RMIMoveGripper, paramIIDLCaDSEV3RMIMoveVertical,
				paramIIDLCaDSEV3RMIMoveHorizontal);
	...
	public synchronized void startGUIRefresh(int paramInt) {
		if ((paramInt > 99) && (this.refreshThread == null)) {
			this.refreshThread = new CaDSRobotGUIRefresh(paramInt, this,
					this.consumer);
			...
			System.out.println(
					"GUI refresh time is not provided! Must be 100 or higher. Is= "
							+ paramInt);
		}
	}
	...
	public void setChoosenService(String paramString, int paramInt1,
			int paramInt2, boolean paramBoolean) {
		this.controller.setChoosenService(paramString, paramInt1, paramInt2,
				paramBoolean);
	}
